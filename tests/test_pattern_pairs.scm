...
; - Matches pairs (cons cells) - TYPE-AWARE
...
; Pair pattern on non-pair value (TYPE CHECK)
(⊨ :pair-fail-not-pair
   #t
   (⚠? (∇ #42 (⌜ (((⟨⟩ x y) x))))))
...
; Check tail is a pair (TYPE PREDICATE)
(⊨ :list-tail-check
   #t
   (∇ (⟨⟩ #1 (⟨⟩ #2 ∅)) (⌜ (((⟨⟩ h t) (⟨⟩? t))))))
...
; Nil vs pair pattern (TYPE MISMATCH)
(⊨ :pair-edge-nil
   #t
   (⚠? (∇ ∅ (⌜ (((⟨⟩ x y) x))))))

; Atom vs pair pattern (TYPE MISMATCH)
(⊨ :pair-edge-atom
   #t
   (⚠? (∇ #42 (⌜ (((⟨⟩ x y) x))))))

; Boolean vs pair pattern (TYPE MISMATCH)
(⊨ :pair-edge-bool
   #t
   (⚠? (∇ #t (⌜ (((⟨⟩ x y) x))))))
